...
class SerialWorker(QThread):
    ...
    def __init__(self, port=None, save_data=True, normalization_low=0, normalization_high=700, csv_path=None):
        ...
        self.save_dir = "Routine_acq_raw_data_6x6"  # 6x6专属保存目录
        self.data_queue = Queue()
        self.writer_thread = None
        self.is_saving = False

        # 滤波器功能（6x6=36个点）
        self.filter_handlers = {i: FilterHandler(fs=100.0, low_cutoff=40.0, order=8) for i in range(36)}
                ...
                if self.ser.in_waiting > 0:
                    ...
                    while start_index != -1:
                        ...
                        if end_index != -1:
                            ...
                            if len(parsed) == 360:  # 6x6=36个点，10帧数据=360个点
                                # 处理10帧数据（10,36）
...
class DataReplayWorker(SerialWorker):
        ...
        try:
            ...
            try:
                ...
                if self.raw_data.ndim != 2 or self.raw_data.shape[1] != 36:
                    raise ValueError(f"CSV格式错误，应为N行36列（6x6），实际为{self.raw_data.shape}")
    ...
    def process_frame(self):
        ...
        # 2. 10帧取平均（批量计算）
        average = np.mean(current_frames, axis=0)  # 形状(36,)
...
class RoutineWaveformVisualizer:
    def __init__(self, parent_widget):
        ...
        self.plot.setYRange(0, 4095)  # 适配16位数据
        ...
        for i in range(9):  # 每行9个点，显示9条曲线
            pen = pg.mkPen(color=colors[i], width=2) 
            curve = self.plot.plot(pen=pen, name=f'点{i+1}')
            self.curves.append(curve)

        # 波形数据缓冲区（每个点300帧历史）
        self.show_data = [[0] * 300 for _ in range(9)]
    ...
    def change_row(self, index):
        ...
        if self.selected_row == 0:
            ...
            self.plot.setTitle(f"第{self.selected_row}行波形（9个点）")
        # 重置当前行的波形数据显示
        self.show_data = [[0] * 300 for _ in range(9)]

    # 更新波形（显示选中行的9个点）
    def update_plot(self, new_row):
        if len(new_row) == 36:  # 6x6=36个点
            if self.show_all_average:
                ...
                # 提取选中行的9个点数据（行优先：第selected_row行的索引为(selected_row-1)*9 ~ selected_row*9 -1）
                row_start_idx = (self.selected_row - 1) * 9
...
class MatrixVisualizer:
    def __init__(self, layout, interplotation=False, rotation_angle=0, flip_horizontal=False, flip_vertical=False,
        ...
        self.current_data = np.zeros((4, 9))  # 4x9矩阵
...
class SerialSelectionDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("选择串口或数据回放（6x6矩阵）")
...
class MainWindow(QMainWindow):
    ...
    def init_main_ui(self):
        self.setWindowTitle("6x6传感器 - 图像与波形双视图")
