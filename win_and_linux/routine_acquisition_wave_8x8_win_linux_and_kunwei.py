...
class SerialWorker(QThread):
    ...
    def write_raw_data_to_file(self):
        ...
        with open(self.csv_path, 'w', newline='', buffering=65536) as csvfile:
            ...
            # 创建按组排列的列标题
            headers = ['Timestamp(ns)']  # 添加时间戳列标题，与Kunwei保持一致
            # 预计算索引映射关系
            index_mapping = []
            ...
            while self.is_saving:
                if not self.data_queue.empty():
                    ...
                    # 获取ns级时间戳（使用perf_counter_ns，系统级高精度时间），与Kunwei保持一致
...
class SerialSelectionDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # 4. 添加控件到布局
        self.layout.addWidget(sensor_label)
    ...
    def load_available_ports(self):
        ...
        for port in available_ports:
            ...
            self.sensor_port_combo.addItem(port_info, port.device)  # 存储真实端口名（如COM3）作为用户数据
    ...
    def on_confirm(self):
        """确认选择，保存串口并关闭对话框"""
        self.selected_sensor_port = self.sensor_port_combo.currentData()  # 获取真实端口名
...
class MainWindow(QMainWindow):
    ...
    def init_main_ui(self):
        ...
        self.right_layout.setSpacing(2)  # 两个波形组件间距2px
