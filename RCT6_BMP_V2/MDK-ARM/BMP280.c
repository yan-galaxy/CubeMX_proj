...
//    status = BMP280_SetMode(dev, BMP280_OSRS_T_2X, BMP280_OSRS_P_16X, BMP280_MODE_NORMAL);
	// 修改后（50Hz配置）：osrs_t=×1，osrs_p=×8，保持正常模式
	status = BMP280_SetMode(dev, BMP280_OSRS_T_1X, BMP280_OSRS_P_8X, BMP280_MODE_NORMAL);
    if (status != HAL_OK) {
        return HAL_ERROR;
    }
//		原代码（10Hz配置）  
...
//    status = BMP280_SetConfig(dev, BMP280_FILTER_4, BMP280_T_STANDBY_62_5MS);
	// 修改后（50Hz配置）：t_standby=0.5ms，保持滤波器系数=4（不影响采样频率）
	status = BMP280_SetConfig(dev, BMP280_FILTER_4, BMP280_T_STANDBY_0_5MS);
