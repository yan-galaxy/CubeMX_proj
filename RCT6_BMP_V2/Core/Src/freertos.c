...
//    bmp_dev4.cs_pin = SPI_CS4_Pin;
	...
	send_fdata.head[0]=0x55;//帧头
	send_fdata.head[1]=0xAA;
	send_fdata.head[2]=0xBB;
	send_fdata.head[3]=0xCC;
	send_fdata.tail[0]=0xAA;//帧尾
	send_fdata.tail[1]=0x55;
	send_fdata.tail[2]=0x66;
	send_fdata.tail[3]=0x77;
...
//			sprintf(usb_TxBuf,"%f,%f\r\n",bmp_dev1.pressure,LowPass_value1);
			...
			send_fdata.value[0] = bmp_dev1.pressure;
			send_fdata.value[1] = bmp_dev2.pressure;
			send_fdata.value[2] = bmp_dev3.pressure;
			send_fdata.value[3] = bmp_dev4.pressure;
			
//			CDC_Transmit_FS((uint8_t *)usb_TxBuf,strlen(usb_TxBuf));
//			CDC_Transmit_FS((uint8_t *)(&Highpass_fdata),24);
			CDC_Transmit_FS((uint8_t *)(&send_fdata),24);
