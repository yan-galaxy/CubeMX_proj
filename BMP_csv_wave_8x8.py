...
class SerialWorker(QThread):
    ...
    def __init__(self, port='COM5', normalization_low=0, normalization_high=5.0):
        super().__init__()
        self.port = port
        ...
        self.matrix_init = []
        self.matrix_init = np.zeros(64)  # 初始化为64元素的numpy数组
        self.matrix_flag = 0
                    ...
                    while start_index != -1:
                        ...
                        if end_index != -1:
                            ...
                            if len(parsed) == 64:
                                ...
                                if self.matrix_flag <= 8:
                                    self.matrix_init += parsed.copy()
                                    self.matrix_flag += 1
                                elif self.matrix_flag == 9:
                                    self.matrix_init += parsed.copy()
                                    self.matrix_init /= 10.0
                                    self.matrix_flag += 1
                                    print("基线矩阵计算完成:", self.matrix_init)
                                else:
                                    ...
                                    self.waveform_ready.emit(result.tolist())
